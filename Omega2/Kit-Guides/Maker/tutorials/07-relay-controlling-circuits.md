...
// TODO: be consistent with capitalization: Relay Expansion, Dock
...
// TODO: need a section that introduces the relay expansion:
...
>In this tutorial, we'll be using the power supplied by our Dock due to easy access. Feel free to try using different power supply methods, but take note you may need voltage dropping resistors for higher powered supplies to avoid burning out the buzzer. // TODO: add a note that you should only try this if you're confident in your abilities
...
* 1x Relay Expansion
* 1x Buzzer
* 1x SPDT (or three-way) switch		// TODO: let's not stray from the labels in the materials lists
* 1x Breadboard
* 7x Jumper wires (M-M)

#### Hooking up the Components

The circuit for this	// TODO: ?, let's have a nicer intro
...
// TODO: explain that black is usually associated with ground
...
1. ( // TODO: this next sentence is wiggity wack, pls edit) Take the jumper connected to the `OUT` terminal and plug that into the row the cathode of your buzzer is plugged into. We have it plugged into row 1 column C.
...
The code we'll be using is a bit more complicated than you may think. We leverage the `relayExp` class from the `OmegaExpansion` Python Module to simplify the operation of the Relay Expansion. For this experiment, it would be reasonable to assume that we will check the state of the SPDT switch and then set the buzzer accordingly. However, we'll actually read the state of the relay  first, make sure the state is different from the switch state, and then switch the Relay if needed. We'll cover why we do this below - but before we do, let's get to the action!
...
// TODO: 'very good habit to get into for fast debugging - since you will be able to use the output of your program to quickly identify the source of your issue'
//	- good practise to include a brief backing/evidence for any claims you make


#### Reading then Writing

// TODO: lol not sure if the word 'innate' really applies to computer programs
...
// TODO: make a note that in this case, telling a relay that's already on to turn on won't harm it, but that it's good practise
...
// TODO: expand on this, go into a bit more detail about how one would use debug messages to debug their program
